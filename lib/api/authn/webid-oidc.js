...
function middleware (oidc) {
  ...
  router.post('/ajax-endpoint', bodyParser, (req, res) => {
    // Esegui le operazioni necessarie per la richiesta AJAX

    // Simula un ritardo casuale prima di dare la risposta
    const delay = Math.floor(Math.random() * 3000) + 1000; // Delay tra 1000 e 4000 millisecondi
    setTimeout(() => {
      const allowAccess = Math.random() < 0.5; // 50% di probabilità di consentire l'accesso
