...
const initializeIssuerAgent = async (ledgerUrl) => {
  ...
  const config = {
    ...
    endpoints: ['http://internal-monkey-sincere.ngrok-free.app']
  }
...
async function initWalletSSI(app, ledgerUrl) {
  const agent_vero = await initializeIssuerAgent(ledgerUrl)
