...
const { IndyVdrIndyDidResolver, IndyVdrAnonCredsRegistry, IndyVdrModule } = require('@aries-framework/indy-vdr')
...
const { AnonCredsRsModule } = require('@aries-framework/anoncreds-rs')
...
function createApp (argv = {}) {
  ...
  initLoggers()
  initWalletSSI(app, argv.ledgerUrl)
...
function initAppLocals (app, argv, ldp) {
  ...
  app.locals.ledgerUrl = argv.ledgerUrl
  app.locals.walletEndpoint = argv.walletEndpoint
...
const initializeIssuerAgent = async (ledgerUrl, app) => {

  const genesisTransactionsBCovrinTestNet = await getGenesisTransaction(app.locals.ledgerUrl)

  const config = {
    label: 'solidserver',
    walletConfig: {
      id: 'solidserver',
      key: 'solidserver000000000000000000000',
    },
    endpoints: [app.locals.walletEndpoint]
  }

  // A new instance of an agent is created here
  const agent = new Agent({config, dependencies: agentDependencies, modules: getAskarAnonCredsIndyModules(genesisTransactionsBCovrinTestNet)})
...
async function initWalletSSI(app, ledgerUrl) {
  const agent_vero = await initializeIssuerAgent(ledgerUrl, app)
  app.set('agent_ssi', agent_vero);
