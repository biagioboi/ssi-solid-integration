...
/**
 * Authenticates a user via SSI.
 */
class SSIAuthenticator extends Authenticator {
  /**
   * @constructor
   * @param options {Object}
   *
  ...
  /**
   * Factory method, returns an initialized instance of SSIAuthenticator
   * from an incoming http request.
   *
   * @param req {IncomingRequest}
   * @param req.connection {Socket}
  ...
  static fromParams (req, options) {
    ...
    return new SSIAuthenticator(options)
  }
  ...
  findValidUser () {
    ...
    .then(foundUser => {
      ...
      return this.accountManager.userAccountFrom(foundUser)
    })
  }
